...
namespace YCIOT.ModbusPoll.Vendor.XAGY
{
    public static class Box
    {
        ...
        public static void Get_XAGY_WTPM_TrunkPressure_Mock(ModbusRtuOverTcp client, RedisClient redisClient, string messageString)
        {
            var par = messageString.FromJson<ControlRequest>();
            var waterStation = new IotDataWaterStation();
            waterStation.DeviceTypeId = par.DeviceTypeId;
                ...
                if (!par.CommandParameter.IsNullOrEmpty())
                {
                    ...
                    if (dic != null)
                    {
                        ...
                        if (read.IsSuccess)
                        {
                            ...
                            var value8 = client.ByteTransform.TransUInt16(read.Content, 8);
                            var value6 = client.ByteTransform.TransUInt16(read.Content, 6);
                            waterWell.CumulativeFlow = (value6 << 16 | value8) / 100.0;//表头累计
